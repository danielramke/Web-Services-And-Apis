package de.lyth.booblemaps.repositories;
...
public class MockAddressRepository {
    ...
    public static Address getRandom() {
        ...
        String randomAddress = ADDRESSES[index];

        String[] parts = randomAddress.split(",");
        if(parts.length < 2) parts = new String[]{"No Addresses, Was found!"};
        String street_and_number = parts[0];
        String city_zipcode = parts[1];

        String[] city_zip_parts = city_zipcode.split(" ");
        LinkedList<String> list = Arrays.stream(city_zip_parts)
                .map(String::trim).collect(Collectors.toCollection(LinkedList::new));

        String zipcode = list.pollLast();
        String state = list.pollLast();
        String city = String.join(" ", list);
        return new Address(street_and_number, city, state, zipcode);
    }
